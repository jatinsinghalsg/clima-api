...
import { UpcomingAGMRequest } from 'src/interfaces/tumelo.interface';

export const upcomingAGM: APIGatewayProxyHandler = async (
  event: APIGatewayEvent
): Promise<APIGatewayProxyResult> => {
  const { TUMELO_URL } = process.env;

  const pathParameter: UpcomingAGMRequest = {
  ...
  try {
    const organizations = await tumelo.upcomingAGM(
      pathParameter
    );
